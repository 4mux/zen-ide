# Zen IDE — User Manual

> **A minimalist and opinionated IDE built with Python and GTK4**

Welcome to the Zen IDE wiki — the complete user manual covering every feature, keyboard shortcut, configuration option, and workflow tip.

**Version:** 0.1.0 · **Platform:** macOS, Linux · **Stack:** Python · GTK4 · GtkSourceView 5 · VTE

---

## Table of Contents

### Getting Started
- [Installation & First Launch](Installation)
- [The Interface at a Glance](Interface-Overview)
- [Quick Start Guide](Quick-Start)

### Core Features
- [Editor](Editor)
- [File Explorer (Tree View)](File-Explorer)
- [Integrated Terminal](Terminal)
- [Keyboard Shortcuts — Complete Reference](Keyboard-Shortcuts)

### AI Integration
- [AI Setup & Providers](AI-Setup)
- [AI Chat](AI-Chat)
- [Inline Code Completions](Inline-Completions)

### Tools & Panels
- [Dev Pad — Activity Tracker](Dev-Pad)
- [Sketch Pad — ASCII Diagrams](Sketch-Pad)
- [Diff View](Diff-View)
- [Widget Inspector](Widget-Inspector)

### Customisation
- [Themes](Themes)
- [Fonts](Fonts)
- [Settings Reference](Settings)
- [Formatters & Linters](Formatters-and-Linters)

### Reference
- [Supported Languages & File Types](Supported-Languages)
- [Language Examples](Language-Examples)
- [User Directory (~/.zen_ide)](User-Directory)
- [Popup Dialogs](Popups)
- [Status Bar](Status-Bar)
- [Code Navigation](Code-Navigation)
- [Autocomplete / IntelliSense](Autocomplete)
- [FAQ & Tips](FAQ)

---

<p align="center"><em>Made with ❤️ in 🇬🇧</em></p>
